...
// Get initial character data from DOM on page load
let initialCharacterData = {};

document.addEventListener('DOMContentLoaded', function() {
    // Store initial character data from preview elements
    initialCharacterData = {
        name: document.getElementById('previewName')?.textContent || '',
        creator: document.getElementById('previewCreator')?.textContent || '',
        description: document.getElementById('previewDesc')?.textContent || '',
        color: document.getElementById('previewColorBox')?.style.backgroundColor || '',
        alignment: document.getElementById('previewAlignment')?.textContent || ''
    };

    // Set initial alignment selection
    const alignmentBadge = document.getElementById('previewAlignment');
    if (alignmentBadge) {
        selectedAlignment = alignmentBadge.textContent.trim();
    }
...
function syncPreview(type, val) {
    ...
    if (!val || val.trim() === '') {
        ...
        if (type === 'color') {
            target.style.backgroundColor = initialCharacterData[type] || '#6366f1';
        } else {
            target.innerText = initialCharacterData[type] || '';
        }
    } else {
        if (type === 'color') {
...
function selectAlignment(alignment, element) {
    ...
    if (previewAlignment) {
        ...
        const currentColor = colorInput ? colorInput.value : initialCharacterData.color || '#6366f1';
...
function resetFile(event) {
    ...
    // Get the original image from the data attribute
    const originalSrc = previewMainImg?.getAttribute('data-original-src') || '';

    if (imgResult) {
        imgResult.src = originalSrc;
    }
    if (previewMainImg && originalSrc) {
        previewMainImg.src = originalSrc;
    }
    
    // Show/hide appropriate containers based on whether there's an original image
    if (originalSrc) {
        if (dropZoneUI) dropZoneUI.classList.add('hidden');
        if (previewContainer) previewContainer.classList.remove('hidden');
    } else {
